 ...
 * @param addurl//目标页面地址
 */
function createDialog(title,url,gridid,infoid,width,height) {
	openDialog(title,url,gridid,infoid,width,height);
}
...
function openDialog(title,url,gridId,infoid,width,height){
...
function openDetailDialog(title,url,gridId,infoid,width,height){
	url=url.replace("{id}",infoid);
	width = width?width:'800px';
	height = height?height:'500px';
	if(navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)){//如果是移动端，就使用自适应大小弹窗
